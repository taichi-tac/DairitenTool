...
class QualityGates:
    ...
    def constitutional_review(self, chapter: Chapter) -> ConstitutionalResult:
...
## 審査ルール
FACT-001: 明らかなハルシネーション（存在しない企業・ツール・統計）がないか
FACT-002: 数値・統計に「（出典: ○○）」または「〜とも言われています」の表現があるか
...
{{"scores": {{{", ".join(f'"{d.split(":")[0].strip().lower()}": 0.0' for d in dimensions)}}}, "feedback": "改善コメント（1文）"}}"""
    ...
    async def score(self, chapter: Chapter) -> QualityScore:
        """3エージェント並列で5次元スコアリング"""
        r1, r2, r3 = await asyncio.gather(
            self._score_by_reviewer(chapter, "事実確認専門家", ["accuracy: 情報の正確性・根拠の明確さ", "brand_safety: ブランド安全性"]),
            self._score_by_reviewer(chapter, "読者体験アナリスト", ["readability: 読みやすさ・初心者向け", "value: 読者が得られる実用的価値"]),
            self._score_by_reviewer(chapter, "コンテンツ独自性審査員", ["originality: 独自視点・他書との差別化"]),
        )
    ...
    def run_gate(self, chapter: Chapter, max_revisions: int = 2) -> tuple[Chapter, QualityScore]:
        """フル品質チェック: constitutional_review → revise(必要時) → score"""
        current = chapter
        for rev in range(max_revisions + 1):
            result = self.constitutional_review(current)
        ...
        score = asyncio.run(self.score(current))
